...
### 3.1 Integrity and availability: an axis orthogonal to location

Resolution and anchoring, as framed above, are both *location* questions — which substrate a document or record is fetched from or committed to. A second axis cuts across both and is as consequential for what a representation commits its implementers to: whether an artifact's **integrity** is bound to its location or to its content.

An artifact that is **content-addressed** — named by a cryptographic digest of its own bytes — carries its integrity independent of where it resolves. A verifier holding the address confirms it received the intended artifact by recomputing the hash, trusting no ledger, domain, or registry to have served honest bytes. Location then governs only two residual properties: **availability** (can the artifact be fetched at all, and who can withhold it) and **freshness** (is this the current artifact, or has it been revoked or superseded).

This reframes the failure mode raised above. "Who can make an agent disappear" is an *availability* property; it is not an *integrity* one, and a representation that models a single undifferentiated substrate conflates them. A content-addressed artifact cannot be silently altered by whoever hosts it — it can only be withheld or allowed to go stale. Governance capture of the resolution layer therefore threatens availability and freshness while leaving integrity intact: a materially different risk surface than capture of a location-authoritative store, where the host can also substitute content.

The append-only, content-hash record model noted in Part I (the one that borrows ledger vocabulary while disclaiming blockchain and consensus) is one point in this space: it obtains tamper-evidence — the integrity property — without a ledger, leaving availability and freshness to be resolved by whatever policy hosts it. The same construction appears in the accountability and belief-state records under discussion in the W3C AI Agent Protocol CG, where a decision record and the belief-state reference it carries are content hashes — resolution-free for integrity, whatever anchors their availability.

Two consequences for the knowledge-representation model. First, the resolvable-artifact set is wider than the identity document: the *evidence a trust decision rested on* — what the agent took to be the case when it acted, distinct from who the agent is — is itself an artifact with a resolution dependency, and one that content-addressing can free of location for integrity. A representation that models identity resolution but not evidence resolution describes who acted without describing on what basis. Second, each modelled artifact needs two attributes, not one: its resolution/anchoring **location**, and whether its **integrity** is content-addressed or location-bound. Term-level crosswalks that record the first and omit the second will, again, align cleanly and interoperate poorly — a content-addressed "proof record" and a registry-authoritative one bearing the same label make incomparable security promises.
